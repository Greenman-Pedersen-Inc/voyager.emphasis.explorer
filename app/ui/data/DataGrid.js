define(
    ...
    function(
        ...
        return function DataGrid(initialFilterParameters, columns, chartContainerID, chartTitleID, columnSelectorID, suffix, chartLabel) {
            const self = this;
            ...
            gridColumns.forEach(column => {
                ...
                columnLink.className = column.hidden ? 'dropdown-item column-selector' : 'dropdown-item column-selector column-visible';
            ...
            this.grid = new gridjs.Grid({
                ...
                server: {
                    url: createCrashDetailsQuery(initialFilterParameters, columns.map(column => column.id)),
                    then: (data) => {
                        // if the server returns no data the result of the map is undefined
                        if (data.results) {
                            return data.results.map(feature => CodeTranslator.convertFeatureObjectValues(feature));
                        } else {
                            return [];
                        }
                    },
                    handle: (res) => {
                        ...
                        if (res.status === 404) { dataObject } // no data returned
                ...
                self.grid.updateConfig({
                    server: {
                        ...
                        handle: (res) => {
                            ...
                            if (res.status === 404) { return dataObject } // no data returned
                            if (res.status === 500) { return dataObject } // no data returned
                            if (res.ok) { return res.json(); }
