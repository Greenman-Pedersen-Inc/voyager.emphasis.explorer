define([
    ...
    function updateHeatmapPaint(map, colorRamp, layerDefinition, clusterAttributes) {
        function createHeatmapScheme(colorRamp) {
            const colorFill = JSON.parse(JSON.stringify(layerDefinition.legend[layerDefinition.legend.type + '-color']));
    ...
    return function SRIClusters(map, sourceID, legend) {
        const self = this;
            ...
            getSRIData(filterParameters)
                .then(sriData => {
                    ...
                    if (sriData.features && sriData.features.length > 0) {
                        let colorRamp = self.legendCard.update(filterParameters, sriData.features, 'visible', true, true, true);
                        let bounds = new mapboxgl.LngLatBounds(sriData.features[0].geometry.coordinates, sriData.features[0].geometry.coordinates);
