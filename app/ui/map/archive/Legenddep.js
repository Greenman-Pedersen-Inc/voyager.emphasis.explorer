define([
    ...
    return function Legend(map) {
        ...
        this.update = function(legendScheme, legendAttributes) {
            ...
            document.querySelectorAll('#crashes-container>div').forEach(element => {
                element.style.display = 'none';
            });
            document.querySelector(`#${legendAttributes.holder}`).parentElement.style.display = 'block';



            // remove current legend items
            legendHolder.classList.add('hidden');
            ...
            if (legendScheme.length == 0) {
                ...
                // need to add the last legend item "to" attribute to complete all the range numbers
                if (legendScheme.length < legendAttributes.colorArray.length) {
                    const item = legendScheme[legendScheme.length - 1];
                    var itemDiv = document.createElement("div");
                    var itemSpan = document.createElement("span");
                    itemSpan.style.backgroundColor = item.color;
                    var itemLabel;
                    itemLabel = document.createTextNode(item.to.toLocaleString("en-US"));
                    itemDiv.appendChild(itemSpan);
                    itemDiv.appendChild(itemLabel);
                ...
                var legendSchemeReverse = legendScheme.reverse();
                for (let index = 0; index < legendSchemeReverse.length; index++) {
                    const item = legendSchemeReverse[index];
                    var itemDiv = document.createElement("div");
                    var itemSpan = document.createElement("span");
                    itemSpan.style.backgroundColor = item.color;
                    var itemLabel;
                    itemLabel = document.createTextNode(item.from.toLocaleString("en-US"));

                    if (legendScheme.length < legendAttributes.colorArray.length) {
                        itemLabel = document.createTextNode(item.from.toLocaleString("en-US"));
                    } else {
                        itemLabel = document.createTextNode(item.from.toLocaleString("en-US") + " - " + item.to.toLocaleString("en-US"));
                    }
                    itemDiv.appendChild(itemSpan);
                    itemDiv.appendChild(itemLabel);
                    legendHolder.appendChild(itemDiv);
            ...
            legendHolder.classList.remove('hidden');
        ...
        this.createLegendScheme = function(layerString, countAttribute, colorArray) {
            ...
            if (features.length > 0) {
                ...
                if (difference >= colorArray.length) {
                    var bucketSize = Math.floor(difference / colorArray.length);
                    // if (bucketSize == 1) bucketSize = colorArray.length - 1;
                    for (var i = 0; i < colorArray.length; i++) {
                        if (i == colorArray.length - 1) {
                            colorRange.push({
                                from: lowerBound,
        ...
        this.createHeatmapLegend = function() {
            ...
            groupCardHeader.id = `card-header-${legendGroup.id}`;
            ...
            groupCardHeaderToggle.setAttribute('aria-controls', `collapse-pane-${legendGroup.id}`);
            groupCardHeaderToggle.setAttribute('data-target', `#collapse-pane-${legendGroup.id}`);
            groupCardHeaderToggle.setAttribute('data-toggle', 'collapse');
            groupCardHeader.append(groupCardHeaderToggle);

            let legendCollapse = document.createElement('div');
            legendCollapse.id = `collapse-pane-${legendGroup.id}`;
        ...
        this.createLegendEntries = function(legendGroup, visible = 'none') {
            ...
            groupCardHeaderToggle.className = 'btn btn-sm legend-group-label';
            groupCardHeaderToggle.type = 'button';
            groupCardHeaderToggle.innerHTML = '&#x25E2 ' + legendGroup.label;
            groupCardHeaderToggle.setAttribute('aria-expanded', 'false');
            groupCardHeaderToggle.setAttribute('aria-controls', `collapse-pane-${legendGroup.id}`);
            groupCardHeaderToggle.setAttribute('data-target', `#collapse-pane-${legendGroup.id}`);
            groupCardHeaderToggle.setAttribute('data-toggle', 'collapse');
                ...
                legendGroup.layers.forEach(layerGroup => {
                    if (layerGroup.layers) {
                        ...
                        layerGroup.layers.forEach(subgroup => {
                            ...
                            // var idName = legendGroup.id + "_" + layerGroup.label + "_" + subgroup.label;
                            var idName = subgroup.id;
                            var mapSourceName = idName + "_src";
                            var mapLayerName = idName;
                            var beforeLayer = null;
                            if (subgroup.beforeLayer) beforeLayer = subgroup.beforeLayer;
                            if (!map.getSource(mapSourceName)) {
                                console.log(mapSourceName)
                                ...
                                map.on('click', mapLayerName, function(e) {
                                    ...
                                    popupDiv.style = 'width: calc(20vw); height: calc(20vh);';
                                    var headerDiv = document.createElement('div');
                                    var htmlString = "<h6>" + legendGroup.label + " - " + layerGroup.label + " (" + subgroup.label + ")</h6>";
                                    headerDiv.innerHTML = htmlString;

                                    var tableDiv = document.createElement('div');
                                    tableDiv.style = 'width: calc(20vw); height: calc(20vh); overflow-y: scroll; overflow-x: auto;';
                        ...
                        var beforeLayer = null;
                        if (layerGroup.beforeLayer) beforeLayer = layerGroup.beforeLayer;
                        ...
                        if (!map.getSource(mapSourceName)) {
                            ...
                            map.on('click', mapLayerName, function(e) {
                                ...
                                var popupDiv = document.createElement('div');
                                popupDiv.style = 'width: calc(20vw); height: calc(20vh); overflow-y: scroll; overflow-x: auto;margin-left: .8rem; margin-right: 0.75em;';
                                var headerDiv = document.createElement('div');
                        ...
                        if (layerGroup.outline) {
                            var outlineAttr = layerGroup.outline;
                            map.addLayer({
                                id: outlineAttr.id,
                                type: outlineAttr.layerType,
                                'source': mapSourceName,
                                'source-layer': layerGroup.source,
                                paint: outlineAttr.paint
                            }, outlineAttr.beforeLayer);
                        }
        ...
        this.addLayerGroup = function(layerIDs) {
        ...
        function createLegendScheme(heatmapLayer) {
            var colorRange = [];
            ...
            var features = self.map.queryRenderedFeatures({ layers: [heatmapLayer] });

            if (features.length > 0) {
                var colorArray = ['#a1dbb2', '#fee5ad', '#faca66', '#f7a541', '#f45d4c'];
                // var colorArray = ['#51bbd6', '#f1f075', '#f28cb1'];
                var minMaxCrashes = minMax(features);
                var min = minMaxCrashes.min;
                var max = minMaxCrashes.max;
                var difference = max - min;
        ...
        function updateCrashLegend(legendScheme, layerType = 'county') {
            var idString = "county-legend";
            ...
            legendHolder.classList.remove('hidden');
            legendHolder.display = 'block';
        }

        function updateHeatmapPaint(legendScheme, layerData) {
            function createHeatmapScheme(legendScheme) {
                var colorFill = ['interpolate', ['linear'],
                    ['get', 'crashes']
                ];
                var legendSchemeReverse = legendScheme.reverse();
                legendSchemeReverse.forEach(item => {
                ...
                if (legendSchemeReverse.length > 1) {
                    colorFill.push(legendSchemeReverse[legendSchemeReverse.length - 1].to);
                    colorFill.push(legendSchemeReverse[legendSchemeReverse.length - 1].color);
                }
                return colorFill;
            }
            ...
            if (legendScheme.length == 0) {
                self.map.setPaintProperty(layerId, 'fill-opacity', 0);
            } else {
                var colorFill = createHeatmapScheme(legendScheme);
                self.map.setPaintProperty(layerId, layerData.type + '-color', colorFill);
                if (layerData.paintAttributes) {
                    Object.keys(layerData.paintAttributes).forEach(attr => {
                        console.log(attr, layerData.paintAttributes[attr]);
                        self.map.setPaintProperty(layerId, attr, layerData.paintAttributes[attr]);
                    });
                } else {
                    self.map.setPaintProperty(layerId, layerData.type + '-opacity', 1);
                }
            }
        }
